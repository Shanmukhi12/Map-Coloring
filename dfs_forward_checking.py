...
def Forwardcheck(states, neighbours, colors, domain):
    ...
    output = 'Success'
    coloredSpaces = list( map(colors.get, currentNeighbors))
    if 'Null' in coloredSpaces:
        coloredSpaces.remove('Null')
    for color in domain[currentState]:
        if color not in coloredSpaces:
            # assign consistent color
            colors[currentState] = color
...
def forwardCheck_heuristic(states, neighbours, colors, domain):
    global backtracksNum
    if all(value != 'Null' for value in colors.values()):
        return "Success"
    ...
    if 'Null' in coloredSpaces:
        coloredSpaces.remove('Null')
    # Use Least Constraint Value heuristic to get the color
    ...
    for color in orderedDomain:
        if color not in coloredSpaces:
            ...
            if not result:
                ...
                # Reduce domain
                domainReduction(color, currentNeighbors, colors)
                output = forwardCheck_heuristic(states, neighbours, colors, domain)
                if output != "Failure":
                    return "Success"
                # Restore domain
                domain = previousDomain
...
#Function to get the minimum chromatic number for the map
...
if(withHeuristic==1):
    ...
    result = forwardCheck_heuristic(states, neighbours, colors, domain)
if result == 'Success':
    print(colors)
